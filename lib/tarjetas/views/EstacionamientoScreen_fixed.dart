...
class _EstacionamientoScreenState extends State<EstacionamientoScreen>
  ...
  /// Mapa {numero_tarjeta â†’ minutos_consumidos} sincronizado desde /api/tarjeta/.
  /// Es la fuente de verdad para calcular el saldo disponible en cada tarjeta.
  Map<int, int> _tiemposTarjeta = {};

  /// Mapa {usuario_id â†’ nombre} para mostrar quiÃ©n registrÃ³ cada estacionamiento.
  ...
  /// IDs de estacionamientos cuya sincronizaciÃ³n con el servidor estÃ¡ en curso.
  /// El polling ignora estos IDs para evitar sobreescribir cambios pendientes.
  final Set<int> _enProceso = {};
  Timer? _pollingTimer;
  bool _isRefreshing = false;

  /// SuscripciÃ³n WebSocket para recibir actualizaciones en tiempo real.
  ...
  /// Polling de fallback: activo solo cuando el WebSocket estÃ¡ caÃ­do.
  static const Duration _fallbackPollingInterval = Duration(seconds: 45);

  /// Debounce para persistir cachÃ©: evita escrituras excesivas a disco.
  Timer? _persistDebounce;

  /// Throttle para reintentos de conexiÃ³n por cambio de red.
  DateTime _ultimoReintento = DateTime(2000);

  /// Debounce para evitar mÃºltiples ciclos resumed en rÃ¡faga.
  DateTime _ultimoResumed = DateTime(2000);

  /// Timer del fallback para evitar agendarlo mÃºltiples veces.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      ...
      case AppLifecycleState.hidden:
        ...
        debugPrint('ðŸ“± Background â€” WS vivo, UI suspendida');
        break;
      case AppLifecycleState.resumed:
        _appEnSegundoPlano = false;
        // Ignorar resumed duplicados en rÃ¡faga (< 2s entre sÃ­)
        final ahora = DateTime.now();
        if (ahora.difference(_ultimoResumed).inSeconds < 2) {
          debugPrint('ðŸ“± Foreground â€” duplicado ignorado');
          break;
        }
        _ultimoResumed = ahora;
        debugPrint('ðŸ“± Foreground â€” restaurando UI');
        _reanudarOperaciones();
        break;
      case AppLifecycleState.inactive:
        // TransiciÃ³n breve (ej: llamada entrante) â€” no hacer nada
        break;
      case AppLifecycleState.detached:
        ...
        _wsConectado = false;
        debugPrint('ðŸ“± Detached â€” WS cerrado');
        break;
    }
  }
  ...
  void _reanudarOperaciones() {
    ...
    if (_hayDatosPendientesUI) {
      ...
      debugPrint('ðŸ”„ UI sincronizada con datos acumulados en background');
    }

    // Si WS no reconectÃ³, activar fallback (solo 1 timer a la vez)
    if (!_wsConectado) {
      _fallbackPendiente?.cancel();
  ...
  /// Si el WS se cae, activa un polling HTTP mÃ¡s lento como fallback.
  void _iniciarWebSocket() {
    final ws = ServicioWebSocket.instancia;

    // Conectar (si ya estÃ¡ conectado, no hace nada)
    ...
    _wsEstadoSub = ws.onEstadoCambio.listen((conectado) {
      ...
      if (conectado) {
        ...
        debugPrint('ðŸŸ¢ WS conectado â€” polling HTTP desactivado');
      } else {
        _wsConectado = false;
        if (!_appEnSegundoPlano) {
          debugPrint('ðŸ”´ WS desconectado â€” activando fallback polling');
          _iniciarFallbackPolling();
        }
      }
    });
  ...
  void _procesarEventoWsEstaciones(WsEvento evento) {
    try {
      if (evento.accion == 'snapshot' && evento.datos is List) {
        // Snapshot completo de estaciones â€” merge protegido
        ...
        if (_estaciones.isNotEmpty) {
          ...
          for (var i = 0; i < lista.length; i++) {
            ...
            if (local != null &&
              ...
              debugPrint(
                'ðŸ›¡ï¸ WS snapshot: protegiendo estaciÃ³n #${local.numero} '
        ...
        // ActualizaciÃ³n de una estaciÃ³n especÃ­fica
        final nuevo = Estacionamiento.fromJson(
          Map<String, dynamic>.from(evento.datos as Map),
        );
        if (_enProceso.contains(nuevo.id)) return;

        // ProtecciÃ³n anti-reversiÃ³n: no liberar si hay tarjeta activa local.
        // El servidor podrÃ­a enviar estado=false por una race condition o
        // snapshot parcial; la liberaciÃ³n real llega con tarjetas/delete.
        ...
        if (actual != null &&
          ...
          debugPrint(
            'ðŸ›¡ï¸ WS update: bloqueando liberaciÃ³n de estaciÃ³n '
      ...
      debugPrint('âš ï¸ Error procesando WS estaciones: $e');
    }
  }
  ...
  void _procesarEventoWsTarjetas(WsEvento evento) {
    try {
      if (evento.accion == 'snapshot' && evento.datos is List) {
        ...
        // Proteger tarjetas cuya estaciÃ³n tiene una operaciÃ³n en curso
      ...
      debugPrint('âš ï¸ Error procesando WS tarjetas: $e');
    }
  }

  /// Polling HTTP lento como fallback cuando el WebSocket no estÃ¡ disponible.
  ...
  /// Refresco silencioso automÃ¡tico (fallback polling): sincroniza estaciones y tarjetas.
  /// Solo se ejecuta cuando el WS estÃ¡ caÃ­do. No descarga /api/tarjeta/ para ahorrar datos.
  ...
  /// Refresco completo manual (botÃ³n): descarga estacionamientos + tarjetas.
  ...
  /// Sincroniza estacionamientos y tarjetas vÃ­a HTTP.
  /// Solo se debe llamar cuando el WebSocket NO estÃ¡ activo (fallback).
  Future<void> _sincronizarTarjetasSilencioso() async {
    ...
    try {
      ...
      // Ignorar IDs que tienen una operaciÃ³n de registro/liberaciÃ³n en curso
      ...
      for (final nuevo in nuevasEstaciones) {
        ...
        // ProtecciÃ³n adicional: no revertir a libre si hay registro local activo.
        if (actual != null &&
            actual.estado == true &&
            nuevo.estado == false &&
            _estacionamientosTarjeta.any((t) => t.estacionId == nuevo.id)) {
      ...
      if (estacionesCambiadas.isEmpty &&
        ...
        // Sin diff: persistir igualmente para mantener cachÃ© fresca.
        unawaited(_persistirCacheCompleto());
        return;
      }
      ...
      setState(() {
        ...
        for (final e in estacionesCambiadas) {
          // Buscar por id o por numero para evitar duplicados por cachÃ© corrupto
      ...
      debugPrint(
        'ðŸ”„ Sync: ${estacionesCambiadas.length} est, +${tarjetasActualizadas.length} tar, -${tarjetasLiberadas.length} lib',
      );
    } catch (e) {
      debugPrint('âš ï¸ SincronizaciÃ³n silenciosa: $e');
    }
  }
  ...
  void _initializeConnectivity() {
    _connectivitySubscription = Connectivity().onConnectivityChanged.listen((
      ...
      debugPrint('ðŸ“¡ Estado de conectividad: $result');
  ...
  void _reintentarConexion() {
    if (!mounted || _appEnSegundoPlano) return;
    // Throttle: mÃ¡ximo una vez cada 15 segundos
    final ahora = DateTime.now();
    if (ahora.difference(_ultimoReintento).inSeconds < 15) return;
    _ultimoReintento = ahora;
    debugPrint('ðŸ”„ Red recuperada â€” reintentando WS...');
    // Prioridad: reconectar WS. Si ya estÃ¡ conectado, no hace nada.
    // Solo sincronizar HTTP si WS sigue caÃ­do despuÃ©s de un breve delay.
    final ws = ServicioWebSocket.instancia;
    if (!ws.conectado) {
      ws.reanudar();
      Future.delayed(const Duration(seconds: 5), () {
  ...
  void _initializeOneSignal() {
    ...
    try {
      ...
      debugPrint('âœ… OneSignal inicializado correctamente');
    } catch (e) {
      debugPrint('âŒ Error al inicializar OneSignal: $e');
    }
  }

  // Manejar clics en notificaciones
  void _handleNotificationClicked(OSNotificationClickEvent event) {
    debugPrint('ðŸ‘† NotificaciÃ³n clickeada: ${event.notification.body}');

    // Procesar la notificaciÃ³n cuando el usuario hace clic
    _processNotification(event.notification);
  }

  // Procesar la notificaciÃ³n y actualizar los datos
  void _processNotification(OSNotification notification) {
    try {
      final additionalData = notification.additionalData;
      debugPrint('ðŸ“¨ Datos adicionales de la notificaciÃ³n: $additionalData');

      // Verificar si la notificaciÃ³n estÃ¡ relacionada con estacionamientos
      if (additionalData != null) {
        ...
        debugPrint(
          'ðŸ” Analizando notificaciÃ³n - Tipo: $tipo, EstacionId: $estacionId, AcciÃ³n: $accion',
        );

        // Si es una notificaciÃ³n relacionada con estacionamientos, actualizar datos
        ...
        // Si no hay datos adicionales especÃ­ficos, pero el cuerpo sugiere actualizaciÃ³n
      ...
      debugPrint('âŒ Error al procesar notificaciÃ³n: $e');
    }
  }

  // Manejar la actualizaciÃ³n de estacionamientos desde notificaciÃ³n
  void _handleEstacionamientoUpdate() {
    debugPrint('ðŸ”„ Actualizando datos por notificaciÃ³n...');

    if (mounted && !_appEnSegundoPlano) {
      // Mostrar mensaje de actualizaciÃ³n
      _showCustomSnackBar(
        'Actualizando informaciÃ³n de estacionamientos...',
        isWarning: true,
      );
  ...
  // Forzar actualizaciÃ³n de datos (llamada desde notificaciones push).
  // Si el WS estÃ¡ activo, los datos ya llegan en tiempo real â€” no hacer HTTP.
  // Solo forzar HTTP si el WS estÃ¡ caÃ­do.
  DateTime _ultimoForceRefresh = DateTime(2000);
  void _forceRefreshData() async {
    if (_appEnSegundoPlano || !mounted) return;
    // Throttle: mÃ¡ximo una vez cada 30 segundos
    final ahora = DateTime.now();
    if (ahora.difference(_ultimoForceRefresh).inSeconds < 30) return;
    _ultimoForceRefresh = ahora;

    if (_wsConectado) {
      // WS activo: los datos ya estÃ¡n actualizados en tiempo real.
      debugPrint('ðŸ“¨ NotificaciÃ³n recibida â€” WS activo, sin HTTP extra');
      return;
    }
    try {
      ...
      await _sincronizarTarjetasSilencioso();
      debugPrint('âœ… Datos actualizados por notificaciÃ³n (fallback HTTP)');
    } catch (e) {
      debugPrint('âš ï¸ _forceRefreshData: $e');
    }
  }
  ...
  Future<void> _guardarRangoPreferencias(String rango) async {
    try {
      ...
      debugPrint('âœ… Rango guardado en preferencias: $rango');
    } catch (e) {
      debugPrint('âŒ Error guardando rango: $e');
      if (mounted && !_appEnSegundoPlano) {
  ...
  void _aplicarRangoEstacionamientos() {
    ...
    try {
      ...
      if (partes.length != 2) {
        _showCustomSnackBar(
          'Formato invÃ¡lido. Use: inicio-fin (ej: 1-10)',
          isError: true,
        );
        return;
      }
  ...
  void _loadUserAndData() async {
    try {
      // Una sola lectura de SharedPreferences para usuario + cachÃ©
      final prefs = await SharedPreferences.getInstance();
      ...
      if (userId == 0 && mounted && !_appEnSegundoPlano) {
        _showCustomSnackBar(
          'Error: Usuario no encontrado. Por favor, inicie sesiÃ³n nuevamente.',
          isError: true,
        );
      }

      // --- Datos de cachÃ©: estacionamientos ---
      ...
      // --- Datos de cachÃ©: tarjetas tiempo (numero â†’ minutos consumidos) ---
      ...
      // --- Datos de cachÃ©: nombres de usuarios ---
      ...
      // Un Ãºnico setState: la pantalla pasa de loading a datos en un solo frame
      if (!mounted) return;
      setState(() {
        _usuario = userId;
        _token = token;
      ...
      if (!tieneCache) {
        // Primera vez sin cachÃ©: descargar todo por HTTP, luego activar WS.
        unawaited(
          Future.wait([
                _fetchAndCacheEstacionamientos(),
                _fetchAndCacheEstacionamientosTarjeta(),
        ...
        // Ya hay cachÃ©: ir directo a WebSocket â€” el snapshot WS traerÃ¡
  ...
  void _verificarTiemposExpiradosAlInicio() async {
    ...
    // Persistir la limpieza en cachÃ©
    unawaited(_persistirCacheCompleto());
    debugPrint(
      'ðŸ§¹ ${expirados.length} tarjetas expiradas limpiadas localmente al inicio',
    );
  }
  ...
  Estacionamiento_Tarjeta? _parseEstacionamientoTarjetaFromJson(dynamic json) {
    try {
      ...
      debugPrint('âŒ Error parseando estacionamiento tarjeta: $e');
      debugPrint('âŒ JSON problemÃ¡tico: $json');
      return null;
    }
  }
  ...
  Future<void> _fetchAndCacheEstacionamientos() async {
    if (_appEnSegundoPlano) {
      debugPrint(
        'â¸ï¸  App en segundo plano - Saltando actualizaciÃ³n de estacionamientos',
      );
      return;
    }
    ...
    try {
      ...
      final estaciones = await fetchEstacionamientos(token: _token).timeout(
        ...
        onTimeout: () {
          debugPrint('â° Timeout al obtener estacionamientos');
      ...
      // Silencioso: actualizaciÃ³n en background, no interrumpir al usuario.
      debugPrint('âš ï¸ Sync estacionamientos: $e');
    }
  }
  ...
  Future<void> _fetchAndCacheEstacionamientosTarjeta() async {
    if (_appEnSegundoPlano) {
      debugPrint(
        'â¸ï¸  App en segundo plano - Saltando actualizaciÃ³n de tarjetas',
      );
      return;
    }
    ...
    try {
      ...
      if (!hasConnection) {
        debugPrint(
          'ðŸ“µ Sin conexiÃ³n - No se pueden obtener estacionamientos tarjeta',
        );
        return;
      }
      ...
      final tarjetas = await fetchEstacionamientoTarjeta(token: _token).timeout(
        const Duration(seconds: 25),
        onTimeout: () {
          debugPrint('â° Timeout al obtener estacionamientos tarjeta');
          throw TimeoutException(
      ...
      // Error silencioso: es una sincronizaciÃ³n en background.
      // No interrumpir al usuario con snackbars por fallos de red temporales.
      debugPrint('âš ï¸ Sync tarjetas: $e');
    }
  }

  /// Descarga /api/tarjeta/ y actualiza el mapa {numero â†’ tiempo consumido}.
  ...
  Future<void> _fetchAndCacheTarjetasTiempo() async {
    ...
    try {
      ...
      debugPrint('âš ï¸ Sync tarjetas_tiempo: $e');
    }
  }
  ...
  Future<void> _liberarEstacionamientoExpirado(int estacionId) async {
    ...
    // esperamos la confirmaciÃ³n del servidor.
    _enProceso.add(estacionId);
    try {
      debugPrint('ðŸ”„ Liberando estacionamiento expirado: $estacionId');
      ...
      try {
        // Solo intentar liberar en servidor si la app estÃ¡ activa y hay conexiÃ³n
        if (!_appEnSegundoPlano) {
          final connectivityResult = await Connectivity().checkConnectivity();
          final hasConnection =
              connectivityResult.isNotEmpty &&
          ...
          if (hasConnection) {
            ...
            debugPrint('âœ… Estacionamiento $estacionId liberado en servidor');
          }
        }
      } catch (e) {
        debugPrint('âš ï¸ Error al liberar en servidor: $e');
      }
  ...
  /// Ãndice de tab â†’ filtro de estado.
  void _cambiarFiltroTab(int index) {
    ...
    // La cachÃ© ya estÃ¡ visible; la sincronizaciÃ³n actualiza silenciosamente.
    if (index == 2) {
  ...
  Future<void> _updateEstacionamientoEstadoLocal({
    ...
    try {
      ...
      debugPrint(
        'âœ… Estado actualizado - Estacionamiento #${estacion.numero}: ${estado ? 'OCUPADO' : 'DISPONIBLE'}',
      );
    } catch (e) {
      debugPrint('âŒ Error al actualizar estado local: $e');
      if (mounted && !_appEnSegundoPlano) {
  ...
  Future<void> _persistirCacheCompleto() async {
    ...
    _persistDebounce = Timer(const Duration(seconds: 2), () async {
      try {
        ...
        debugPrint('âš ï¸ Error al persistir cachÃ©: $e');
      }
    });
  }
  ...
  /// Minutos consumidos segÃºn el servidor (/api/tarjeta/). Fuente de verdad.
  /// Fallback local si aÃºn no se descargÃ³ el catÃ¡logo de tarjetas.
  ...
  List<Widget> _buildReservadoContent(Estacionamiento estacion) {
    ...
    // Extraer nombre del reservante de parÃ©ntesis: "AVDA X (Juan PÃ©rez)"
    String? reservadoPor;
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Si no hay info extra, mostrar mensaje genÃ©rico
  ...
  Widget _buildEstacionCard(Estacionamiento estacion) {
    ...
    // Paleta de colores mÃ¡s imponente
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          // â”€â”€ Encabezado con gradiente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          Container(
          ...
          // â”€â”€ Contenido â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          InkWell(
            ...
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // â”€â”€ Reservado: info detallada â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  if (estaDeshabilitado) ...[
                    // Extraer nombre del reservante de los parÃ©ntesis
                    ..._buildReservadoContent(estacion),
                  ],

                  // â”€â”€ VehÃ­culo ocupado â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  if (ocupado &&
                      !estaDeshabilitado &&
                  ...
                  // â”€â”€ Horario â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  if (ocupado &&
                  ...
                  // â”€â”€ BotÃ³n Liberar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  ...
                  // â”€â”€ Libre: hint de toque â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  if (!ocupado && !estaDeshabilitado) ...[
  ...
  void _showRegistroForm(BuildContext context, Estacionamiento estacion) {
    if (_estaDeshabilitado(estacion)) {
      _showCustomSnackBar(
        'Este estacionamiento no estÃ¡ disponible para uso',
        isError: true,
      );
      return;
    }
    ...
    if (_usuario == null) {
      _showCustomSnackBar(
        'Error: Usuario no disponible. Reinicie la aplicaciÃ³n.',
        isError: true,
      );
      return;
    }
    ...
    int minutosRestantesTarjeta =
        120; // se actualiza al escribir el nÃºmero de tarjeta
          ...
          return Dialog(
            ...
            child: Container(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  // â”€â”€ CABECERA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  Container(
                    width: double.infinity,
                  ...
                  // â”€â”€ CONTENIDO SCROLLABLE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  Flexible(
                  ...
                  // â”€â”€ BOTONES FIJOS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                  Container(
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: errorModal != null
                                  ...
                                  : () {
                                      ...
                                      // Capturar datos antes de cerrar el diÃ¡logo
                                      ...
                                      setState(() {
                                        ...
                                        // ActualizaciÃ³n optimista del saldo
                                        final previo =
                                      ...
                                      // 2. Cerrar diÃ¡logo al instante
                                      Navigator.pop(context);
                                      _showCustomSnackBar(
                                        'âœ… Estacionamiento #${estacionCapturado.numero} registrado',
                                      );

                                      // 3. Sincronizar con el servidor en background
                                      unawaited(() async {
                                        try {
                                          // Ejecutar registro de tarjeta y actualizaciÃ³n de estaciÃ³n
                                          // en PARALELO para que el broadcast WS salga mÃ¡s rÃ¡pido
                                          await Future.wait([
                                          ...
                                          // Actualizar tiempo de tarjeta despuÃ©s
                                          ...
                                          if (mounted) {
                                            ...
                                            _showCustomSnackBar(
                                              'âŒ Error al sincronizar con el servidor: $e',
                                              isError: true,
                                            );
                                          }
                                        }
        ...
        validator: (value) {
          ...
          if (label == "Placa (ABC1234)") {
            ...
            if (!regex.hasMatch(upper)) {
              return 'Formato invÃ¡lido. Use 3 letras + 4 nÃºmeros (ABC1234)';
            }
          }
          if (label == "NÃºmero de Tarjeta") {
            if (value!.isEmpty) {
              return 'NÃºmero de tarjeta requerido';
            }
      ...
      child: TextFormField(
        ...
        validator: (value) {
          ...
          if (!regex.hasMatch(value)) {
            return 'Formato invÃ¡lido (HH:mm:ss)';
          }
          return null;
        },
      ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text(
                    'Muestra el estado de ocupaciÃ³n de los espacios de estacionamiento y permite gestionar el ingreso y salida de vehÃ­culos.',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // â”€â”€ Banner resumen de ocupaciÃ³n â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          if (_estaciones.isNotEmpty)
            Builder(
              builder: (_) {
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: "Buscar por nÃºmero de estacionamiento...",
...
// Tiene su propio Timer de 1 segundo â†’ solo se reconstruye Ã©l mismo,
// NO la pantalla completa, eliminando el jank de la versiÃ³n anterior.
// ---------------------------------------------------------------------------
...
class _CountdownTickerState extends State<_CountdownTicker> {
  ...
  void initState() {
    ...
    if (!_expired) {
      ...
      // Tiempo ya expirado al montar el widget (ej: app abierta despuÃ©s de expirar).
